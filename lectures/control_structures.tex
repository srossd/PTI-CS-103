...
In general, it is very common to have a conditional block of code with 1 \ic{if} statement, 1 or more \ic{else if} statements, and 1 \ic{else} statement.

Suppose you have code where multiple of the \ic{if} and \ic{else if} statements are true. In this case, only the \emph{first} true statement will count. For example, consider the following generic code.
...
\emph{Answer}: The code will print ``apple" and then ``dragonfruit". Notice that ``banana" does not get printed even though \ic{B} is true. This is because once one of the \ic{if} or \ic{else if} conditions is true, no other conditions will be checked.

 ``dragonfruit" is always printed because it is outside of the \ic{if}/\ic{else if}/\ic{else} block.
\end{example}
