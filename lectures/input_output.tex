\chapter{Input and Output}\label{ch:io}
...
Using \ic{System.out.println} is one way to
produce output. Here \ic{System.out} refers to the \emph{output stream}
...
As you might imagine, there are many ways to produce output in Java. For example, we can use print to \ic{System.out} with different formatting. We can also output image files that
...
\ic{System.out.println} does not require a String to be provided,
...
The \ic{\textbackslash} is used to create various types of ``escape characters" that the computer treats in a special way. Note that instead of printing ``\textbackslash n", it replaced the \ic{\textbackslash n} with a newline. Another example of an escape character is ``\textbackslash t" for inputting a tab, as demonstrated here. 
