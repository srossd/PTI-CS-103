...
We will return to what the code means shortly, but first let's learn how to run it!
...
Again, note that the file name for \ic{.java} files needs to match the name of the class (for us, the name of our class is \ic{HelloWorld}).
...
As a final exercise, then, type \ic{run HelloWorld} after the final \ic{>} and
...
need to know is that every Java program needs a \ic{main} method, and that when we run a Java program, the code within the \ic{main()} method is executed. 

Inside the \ic{main()} method, we use the \ic{println()} method to print out
whatever is specified between the open parenthesis \ic{(} and closed
parenthesis\ic{ )} that follows immediately after. A semi-colon is always needed after statements, similar to how periods are needed to end sentences in English. This is the meaning of \ic{System.out.println("Hello World!");}
...
class HelloWorld{public static void main(String[] args)
    {System.out.println("Hello World!");}}
\end{code}
Try removing whitespaces in your code, for example so that it looks like the above example, then compile and run it. It should still print \ic{Hello World!} as its output.
